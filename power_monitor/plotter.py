...
class PowerPlotter:
    ...
    def _plot_power_draw(self, ax, timestamps, df):
        ...
        if len(power_draw) > 0:
            ...
            ax.axhline(y=high_threshold, color="red", linestyle="--", linewidth=1, alpha=0.5)
    ...
    def _annotate_high_power_events(self, ax, timestamps, power_draw, threshold):
        ...
        for idx in high_power_indices:
            ax.axvline(x=timestamps[idx], color="orange", linestyle=":", linewidth=1, alpha=0.6)
    ...
    def _plot_cpu_usage(self, ax, timestamps, df):
        ...
        ax.axhline(
            y=80, color="red", linestyle="--", linewidth=1, alpha=0.5, label="High Usage (80%)"
        )

        # Shade regions where CPU usage is very high
        ax.fill_between(timestamps, 0, cpu_usage, where=(cpu_usage >= 80), alpha=0.3, color="red")
